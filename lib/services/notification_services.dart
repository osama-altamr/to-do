...
class NotifyHelper {
  ...
  tz.TZDateTime _nextInstanceOfTenAM(
    ...
    final tz.TZDateTime formattedLocal =
        tz.TZDateTime.from(formatted, tz.local);
  tz.TZDateTime scheduledDate =
        tz.TZDateTime(tz.local, formattedLocal.year, formattedLocal.month, formattedLocal.day, hour, minutes);
    ...
    if (scheduledDate.isBefore(now)) {
      ...
      if (repeat == 'Weekly') {
        scheduledDate = tz.TZDateTime(
            tz.local, now.year, now.month, (formatted.day) + 7, hour, minutes);
      }
      if (repeat == 'Monthly') {
        scheduledDate = tz.TZDateTime(tz.local, now.year, (formatted.month) + 1,
            formatted.day, hour, minutes);
      }
      scheduledDate = afterRemind(remind, scheduledDate);
    }
